...
2. **Truncate large po/XX.po.pending file**: If the `po/XX.po.pending` file
   is too large to load at once, truncate it to process in small batches
   (e.g., batches of 50 entries):

   ```shell
   # Adjust the number 50 as needed based on file size
   awk '/^msgid / && ++count > 50 {exit} 1' po/XX.po.pending > po/XX.po.pending.tmp
