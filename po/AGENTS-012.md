...
When asked to translate `po/XX.po` or similar, follow the steps below. You
must translate every untranslated and fuzzy entry. Step 7 describes the
repeat loop and when the task is complete. **Do not stop before then.**
...
1. **Extract entries to translate and record initial state**: Use `msgattrib`
   ...
   **NOTE**: When using `msgattrib` to extract files, you **MUST** use
   ...
   po_extract_pending () {
       ...
       rm -f po/XX.po.untranslated po/XX.po.fuzzy
   }
   # Run the extraction (do not execute the commands above one by one)
   po_extract_pending
   ```

   If `po/XX.po.pending` is empty, skip to the last step (clean up) as the
...
2. **Check file size and truncate with dynamic batch size**: **BEFORE
   ...
   truncate_as_needed () {
       ...
       then
           ...
           awk -v num="$NUM" '/^msgid / && count++ > num {exit} 1' \
               po/XX.po.pending |
...
7. **Repeat steps 1–6 until `po/XX.po.pending` is empty**
   ...
   Follow this procedure: repeat steps 1–6 (extract, truncate if needed,
   translate, validate, merge) until `po/XX.po.pending` is empty (no entries
   left except the header). Then run steps 8–9 (final verification and
   cleanup). Do not stop early or report partial completion.
...
9. **Clean up**: Only after confirming that translation is complete (step 9
