...
The root object has three main fields:
...
## 3. Entry Object Schema

Each element of `entries` is an object with the following fields.
...
**Optional extension (implementation may add later):**

- `flags`: array of strings parsed from the `#, ...` line (e.g. `["fuzzy", "c-format"]`). If present, `fuzzy` can be derived as `flags` contains `"fuzzy"`. Omitted in the initial version to keep the schema small.
...
| entries[]         | Array of objects: msgid, msgstr, optional msgid_plural/msgstr_plural, comments[], fuzzy |
